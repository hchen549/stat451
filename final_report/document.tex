...
\begin{document}
	...
	However, we were expecting Random Forest to outperform other fitted models while it turns out to be the least accurate model excluding KNN and Decision Tree. It is possible that we did not find the best parameter for Random Forest. The reason is that Random Forest is computationally expensive. Though the performance of Random Forest should be increasing as higher value of parameter \textit{max\_depth} is assigned, the grid search process took hours to run and selected \textit{none} as the best parameter value for \textit{max\_depth}.We are curious to see the performance of Random Forest if we are able to find the best parameter for \textit{max\_depth} with either a enhanced hardware or cloud computing.
...
\section{Conclusion}
	...
	From Table 1, we cconclude that Logistic Regression performs the bestamong all fitted models, with an overall 89.167\% accuracy and Bagging size of 60. Nonetheless, it is possible that Random Forest could be improved during parameter tunning stage
